                    ...
                    <a href="{{ route('shop.category', $cat->slug) }}" class="widget-collection type-space-2 hover-img">
                        <div class="collection_image img-style">
                            @if($cat->thumbnail_image)
                                {{-- thumbnail_image is the dedicated homepage tile image, independent
                                     of banner_image (the category page hero) — no fallback between the
                                     two, so setting one never affects the other. --}}
                                <img class="lazyload" src="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 1 1'%3E%3C/svg%3E" data-src="{{ \Illuminate\Support\Facades\Storage::url($cat->thumbnail_image) }}" alt="{{ $cat->name }}">
                            @else
